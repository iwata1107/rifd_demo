...
struct InventoryMasterListView: View {
    ...
    var filteredMasters: [InventoryMaster] {
        if searchText.isEmpty {
            return inventoryViewModel.inventoryMasters
        } else {
            return inventoryViewModel.inventoryMasters.filter { master in
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                } else if inventoryViewModel.inventoryMasters.isEmpty {
                    VStack(spacing: 20) {
                        ...
                        Button(action: {
                            Task {
                                await inventoryViewModel.fetchInventoryMasters()
                            }
                        }) {
                            Text("再読み込み")
                                .padding()
            ...
            .alert(isPresented: Binding<Bool>(
                get: { inventoryViewModel.error != nil },
                set: { if !$0 { inventoryViewModel.error = nil } }
            )) {
                Alert(
                    title: Text("エラー"),
                    message: Text(inventoryViewModel.error?.localizedDescription ?? "不明なエラーが発生しました"),
